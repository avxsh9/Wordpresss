<?php
/**
 * TAE_CPT — Handles CPT and Taxonomy Registration
 */
...
class TAE_CPT {
    ...
    public function register_meta_boxes() {
        add_meta_box( 'tae_listing_meta', 'Listing Details', array( $this, 'render_meta_box' ), 'event_listing', 'normal', 'high' );
    }

    public function render_meta_box( $post ) {
        wp_nonce_field( 'tae_save_listing', 'tae_listing_nonce' );
        $fields = array(
            'tae_master_id' => array( 'label' => 'Master Event ID', 'type' => 'number' ),
            'tae_price'     => array( 'label' => 'Price (₹)', 'type' => 'number' ),
            'tae_seats'     => array( 'label' => 'Seats / Stand Info', 'type' => 'text' ),
            'tae_date'      => array( 'label' => 'Date (Seller Set - Movie)', 'type' => 'date' ),
            'tae_time'      => array( 'label' => 'Time (Seller Set - Movie)', 'type' => 'time' ),
            'tae_venue'     => array( 'label' => 'Venue (Seller Set - Movie)', 'type' => 'text' ),
    ...
    public function save_meta_boxes( $post_id ) {
        if ( ! isset( $_POST['tae_listing_nonce'] ) || ! wp_verify_nonce( $_POST['tae_listing_nonce'], 'tae_save_listing' ) ) return;
    ...
    public function template_redirect( $template ) {
        if ( is_singular( 'event_listing' ) ) {
            $new_template = TAE_DIR . 'templates/single-event_listing.php';
