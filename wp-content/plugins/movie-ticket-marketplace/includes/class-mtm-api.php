<?php
/**
 * MTM_API — Handles REST API Endpoints
 */
...
class MTM_API {
    ...
    public function register_routes() {
        register_rest_route( self::NS, '/movies', array(
            'methods'             => 'GET',
        ...
        register_rest_route( self::NS, '/movies/(?P<id>\\d+)', array(
            'methods'             => 'GET',
    ...
    public function get_listing( $request ) {
        ...
        if ( ! $post || $post->post_type !== 'movie_tickets' ) {
    ...
    public function create_listing( $request ) {
        ...
        $fields = array( 'mtm_location', 'mtm_datetime', 'mtm_seat_info', 'mtm_price', 'mtm_quantity' );
        foreach ( $fields as $field ) {
            $key = str_replace( 'mtm_', '', $field );
            if ( isset( $params[ $key ] ) ) {
                update_post_meta( $post_id, $field, sanitize_text_field( $params[ $key ] ) );
            }
        }
        update_post_meta( $post_id, 'mtm_status', 'Available' );
        update_post_meta( $post_id, 'mtm_seller_id', get_current_user_id() );
    ...
    private function format_listing( $post ) {
        return array(
            ...
            'location'  => get_post_meta( $post->ID, 'mtm_location', true ),
            'datetime'  => get_post_meta( $post->ID, 'mtm_datetime', true ),
            'seat_info' => get_post_meta( $post->ID, 'mtm_seat_info', true ),
            'price'     => (float) get_post_meta( $post->ID, 'mtm_price', true ),
            'quantity'  => (int) get_post_meta( $post->ID, 'mtm_quantity', true ),
            'status'    => get_post_meta( $post->ID, 'mtm_status', true ),
            'seller_id' => (int) get_post_meta( $post->ID, 'mtm_seller_id', true ),
