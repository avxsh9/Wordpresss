...
<main id="main">
    <?php if (have_posts()) : while (have_posts()) : the_post(); 
        $event_id = get_the_ID();
        $date = get_post_meta($event_id, 'sport_date', true);
        $time = get_post_meta($event_id, 'sport_time', true);
        $location = get_post_meta($event_id, 'venue', true);
        $image = get_post_meta($event_id, 'match_poster', true) ?: get_the_post_thumbnail_url($event_id, 'full');
        if (!$image) $image = 'https://images.unsplash.com/photo-1540039155733-5bb30b53aa14?auto=format&fit=crop&w=1200&q=80';
    ?>
